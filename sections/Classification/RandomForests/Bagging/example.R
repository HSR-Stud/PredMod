...
# init vectors
pred.class = matrix(NA, nrow=B, ncol=n)
...
for (b in 1:B)
{
	...
	cur_tree = tree(AHD~., data = cur_sample, control = tc, method = "gini")
	
	# predict the complete data on the current tree
	pred.class[b,] = predict(cur_tree, newdata=heart, type="class")
	
	# loop over all observations and compute OOB error
	for (i in 1:n)
	{
		oob_list = lapply(oob_samples, FUN = function(x) {match(i,x)})
...
# display results
plot(1:B, oob.error, type = "l", col = "darkcyan", ylim = c(0.1, 0.3), ylab = "")
lines(1:B, test.error, col = "darkred")
grid()
legend("bottomright", legend = c("test error", "OOB error"), col = c("darkred", "darkcyan"), lty = 1)